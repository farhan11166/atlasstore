...
function toggleMode() {
  ...
  document.getElementById("auth-title").textContent = isRegisterMode
    ? "Create account"
    : "Welcome back";
  document.getElementById("auth-subtitle").textContent = isRegisterMode
    ? "Start storing your files"
    : "Sign in to your storage vault";
  document.getElementById("auth-btn-text").textContent = isRegisterMode
    ? "Create Account"
    : "Sign In";
...
function setAuthLoading(loading) {
  document.getElementById("auth-btn-text").style.display = loading
    ? "none"
    : "inline";
  document.getElementById("auth-btn-loader").style.display = loading
    ? "inline-block"
    : "none";
...
function showSkeleton(visible) {
  document.getElementById("skeleton-list").style.display = visible
    ? "flex"
    : "none";
...
async function uploadFile(file) {
  ...
  try {
    // 1. Init Upload (Tell backend we are starting)
    let res = await fetch(API + "/multipart", {
      method: "POST",
      headers: {
    ...
    for (let i = 0; i < totalParts; i++) {
      ...
      const chunkBlob = file.slice(start, end); // Slice the file in browser!

      // Fire off a fetch request for this specific chunk
      const p = fetch(API + `/multipart/${upload_id}/${i}`, {
        method: "POST",
        headers: { Authorization: "Bearer " + token },
        body: chunkBlob,
      }).then((r) => {
        ...
        // Update progress bar as chunks finish
    ...
    // Wait for all chunk HTTP requests to finish
    await Promise.all(uploadPromises);

    // 3. Complete Upload (Tell backend to stitch metadata together)
    res = await fetch(API + `/multipart/${upload_id}/complete`, {
      method: "POST",
      headers: { Authorization: "Bearer " + token },
    });
